...
import cupboard.convert.EntityConverter.Column;
import cupboard.convert.EntityConverter.ColumnType;
import cupboard.internal.IndexStatement;
import cupboard.internal.IndexStatement.Builder;
...
public class DatabaseCompartment extends BaseCompartment {
    ...
    /**
     * Drop all tables for the classes registered with {@link cupboard.Cupboard#register(Class)}
     */
    ...
    public <T> boolean delete(T entity) {
        @SuppressWarnings("unchecked") Class<T> clz = (Class<T>) entity.getClass();
...
//        return true;
    ...
    public void clearTable(Class<?> entity) {
        EntityConverter<?> converter = mCupboard.getEntityConverter(entity);
        mDatabase.execSQL("DELETE FROM " + quoteTable(converter.getTable()));
    }
    ...
    public <T> boolean updateInsert(T entity) {
        EntityConverter<T> converter = (EntityConverter<T>) getConverter(entity.getClass());
        ContentValues values = new ContentValues();
        converter.toValues(entity, values);

        return updateInsert(converter, values);

    }

    public boolean updateInsert(Class<?> entityClass, ContentValues values) {
        EntityConverter<?> converter = getConverter(entityClass);
        return updateInsert(converter, values);

    }

    private boolean updateInsert(EntityConverter<?> converter, ContentValues values) {
        String whereClause = "";
        ...
        List<Column> columns = converter.getColumns();
        for (Column col : columns) {
            ...
            if (index != null) {
                String name = col.name;
                args.add(values.getAsString(name));
                indexCount++;
                if (indexCount == 1) sb.append(name).append(" = ?");
                else sb.append(" AND ").append(name).append(" = ?");
            }
        }
        ...
        int updated = mDatabase.update(quoteTable(converter.getTable()), values, whereClause, whereArgs);
        if (updated != 0) return true;
        if (mDatabase.insertOrThrow(quoteTable(converter.getTable()), null, values) == -1) return false;
