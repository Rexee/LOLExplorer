...
public interface EntityConverter<T> {
    /**
     * Create an entity from the cursor. The cursor supplied is guaranteed to provide the columns in the order returned by {@link cupboard.convert.EntityConverter#getColumns()},
     * but the number of columns might be less if the result does not contain them.
     * <p/>
     * For example, if the converter has 10 columns and the cursor has only 7, the columns 0-6 from {@link cupboard.convert.EntityConverter#getColumns()} will be supplied, even
     * if the original cursor does not contain all of them. This allows a {@link cupboard.convert.EntityConverter} to iterate over the columns without checking for column name.
     * <p/>
     ...
     * Generally speaking do not add content values for columns that aren't returned from {@link #getColumns()} and omit columns of value {@link cupboard.convert.EntityConverter.ColumnType#JOIN}
     *
    ...
    public static class Column {
         ...
         * @see cupboard.annotation.Index
         * @see cupboard.convert.IndexBuilder
         */
